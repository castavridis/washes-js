...
| `engine/` | The `washes` npm-shaped package, v1.12.1 — `src/` (core, GPU sim, timeline sidecar, typings, shader), `tests/`, `dist/washes.standalone.js` for classic script tags |
...
- **`engine/CHANGELOG.md`** tracks the engine package (0.98.0 → 1.12.1).
- **The playground's in-app changelog** (docs panel) tracks the demo itself
  (v0.1 → v1.0.16). A demo release that embeds a new engine says so — e.g.
