...
// ─── Global attachment ──────────────────────────────────────────────────────
  ...
  function createInstance(targetEl, options) {
    ...
    function paintAt(gx, gy, gridRadius, pigmentIdx, strength) {
      // v1.16 — pigment and strength are optional per the declarations;
      // resolve/default exactly as paintNorm does (undefined pigment →
      // the current brush ink, names → indices, strength → 0.5).
      // Previously an omitted pigment fell through to g[undefined] and
      // threw, and an omitted strength poisoned the deposit with NaN.
    ...
    function loop() {
      ...
      if (runSim) {
        if (useGpuSim && gpuSimHandle) {
          ...
          // v1.16 — step through the backend seam (CPU adapter today;
          // a worker or GPU backend slots in behind the same interface).
          // Params are informational for the CPU adapter — simStep reads
          // closure state — but real for future backends.
          const _stepParams = _buildGpuSimParams();
          _simBackend.step(_stepParams);
          _simBackend.step(_stepParams);
        }
      }
