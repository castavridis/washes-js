...
const PARTS = ['pigment-data', 'sim-core', 'sim-backend'];

let lib = fs.readFileSync(LIB_PATH, 'utf8');
const original = lib;

for (const name of PARTS) {
  const partPath = path.join(PARTS_DIR, name + '.part.js');
  if (!fs.existsSync(partPath)) fail(`missing part file: src/parts/${name}.part.js`);
  let content = fs.readFileSync(partPath, 'utf8');
  if (content.endsWith('\n')) content = content.slice(0, -1);

  const beginMark = `    // >>> PART ${name} BEGIN`;
  const endMark = `    // <<< PART ${name} END`;
  const b = lib.indexOf(beginMark);
  const e = lib.indexOf(endMark);
  if (b < 0 || e < 0 || e < b) fail(`sentinels for part "${name}" missing or malformed in washes.js`);
