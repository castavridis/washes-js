...
import { modeConstants, depositFactor } from './brush-texture-deposit.js';
...
for (const mode of modes) {
  ...
  const consts = { amount: DRYNESS, baseThresh: c.baseThresh, bandHalf: c.bandHalf, paperWeight: c.paperWeight, bristleK };
  ...
  for (let y = GY - R; y <= GY + R; y++) {
    for (let x = GX - R; x <= GX + R; x++) {
      ...
      const predicted = depositFactor(fn, ph, consts);
